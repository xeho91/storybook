...
* `argTypes`- Storybook's [argTypes](../api/argtypes.mdx) allow you to customize and fine-tune your stories [`args`](./args.mdx).
...
It is useful to ensure that the story remains a “pure” rendering of the component under test, and any extra HTML or components don't pollute that. In particular the [Source](../writing-docs/doc-block-source.mdx) Doc Block works best when you do this.
...
We can also set a decorator for **all stories** via the `decorators` export of your [`.storybook/preview.js`](../configure/overview.mdx#configure-story-rendering) file (this is the file where you configure all stories):
