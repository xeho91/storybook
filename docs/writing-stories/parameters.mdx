...
<CodeSnippets path="component-story-custom-params.md" />
...
We can also set the parameters for **all stories** via the `parameters` export of your [`.storybook/preview.js`](../configure/overview.mdx#configure-story-rendering) file (this is the file where you configure all stories):

{/* prettier-ignore-start */}

<CodeSnippets path="preview-story-custom-params.md" />
...
If you are defining an API that relies on parameters (e.g. an [**addon**](../addons/introduction.mdx)) it is a good idea to take this behavior into account.
