...
Loaders (experimental) are asynchronous functions that load data for a story and its [decorators](./decorators.mdx). A story's loaders run before the story renders, and the loaded data injected into the story via its render context.
...
We can also set a loader for **all stories** via the `loaders` export of your [`.storybook/preview.js`](../configure/overview.mdx#configure-story-rendering) file (this is the file where you configure all stories):
...
If you're interested in contributing to this feature, read our [contribution guide](../contribute/how-to-contribute.mdx) and submit a pull request with your work.
