...
Learn how and why to write stories in [the introduction](./introduction.mdx#using-args). For details on how args work, read on.
...
<CodeSnippets path="my-component-argtypes-with-mapping.md" />
...
<details>
  ...
  If you are [writing an addon](../addons/writing-addons.mdx) that wants to read or update args, use the `useArgs` hook exported by `@storybook/api`:
...
<details>
  ...
  In Storybook 5 and before we passed the context as the first argument. If you’d like to revert to that functionality set the `parameters.passArgsFirst` parameter in [`.storybook/preview.js`](../configure/overview.mdx#configure-story-rendering):
