...
<Callout variant="info">
  Writing stories directly in MDX was deprecated in Storybook 7. Please reference the [previous documentation](../../../release-6-5/docs/writing-docs/mdx.mdx) for guidance on that feature.
</Callout>
...
If you need to match the component documentation to an existing story, you can configure the [`Meta`](../api/doc-block-meta.mdx) Doc Block to control how the documentation gets rendered. Out of the box, it allows you to define a custom title or a reference to the story you need to document (i.e., via the `of` prop). For example:
...
<Callout variant="info" icon="💡">
  If you're overriding an existing auto-generated documentation page enabled via [`tags`](./autodocs.mdx#setup-automated-documentation) configuration property, we recommend removing it to avoid errors.
</Callout>

Once the custom MDX documentation is loaded, Storybook will infer the title and location using the same heuristic rules to generate [auto-title stories](../configure/sidebar-and-urls.mdx#csf-30-auto-titles) and render it in the sidebar as a `Docs` entry.
...
<CodeSnippets path="storybook-fallback-mdx-install.md" />
...
<CodeSnippets path="main-config-features-legacy-mdx-1.md" />
