...
As a result, for example, customizing the [`Source`](../api/doc-block-source.mdx) block via parameters (see next section) will also affect the Source blocks rendered as part of [`Canvas`](../api/doc-block-canvas.mdx) blocks.
...
For example, you can filter out the `style` prop from all [`Controls`](../api/doc-block-controls.mdx) tables through your Storybook:
...
### [ArgTypes](../api/doc-block-argtypes.mdx)
...
### [Canvas](../api/doc-block-canvas.mdx)
...
### [ColorPalette](../api/doc-block-colorpalette.mdx)
...
### [Controls](../api/doc-block-controls.mdx)

<Callout variant="info">
  Accepts parameters in the namespace `parameters.docs.controls`.
</Callout>

The `Controls` block can be used to show a dynamic table of args for a given story, as a way to document its interface, and to allow you to change the args for a (separately) rendered story (via the [`Story`](../api/doc-block-story.mdx) or [`Canvas`](../api/doc-block-canvas.mdx) blocks).

![Screenshot of Controls block](../api/doc-block-controls.png)

### [Description](../api/doc-block-description.mdx)
...
### [IconGallery](../api/doc-block-icongallery.mdx)
...
### [Markdown](../api/doc-block-markdown.mdx)
...
### [Primary](../api/doc-block-primary.mdx)

The `Primary` block displays the primary (first defined in the stories file) story in a [`Story`](../api/doc-block-story.mdx) block. It is typically rendered immediately under the title in a docs entry.

![Screenshot of Primary block](../api/doc-block-primary.png)

### [Source](../api/doc-block-source.mdx)
...
### [Unstyled](../api/doc-block-unstyled.mdx)
...
Storybook also provides a [`useOf` hook](../api/doc-block-useof.mdx) to make it easier to create your own blocks that function like the built-in blocks.
