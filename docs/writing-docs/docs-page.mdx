...
When you install [Storybook Docs](https://storybook.js.org/addons/@storybook/addon-docs), DocsPage is the zero-config default documentation that all stories get out of the box. It aggregates your [stories](../get-started/whats-a-story.mdx), text descriptions, docgen comments, [args tables](./doc-blocks/doc-block-argstable.mdx), and code examples into a single page for each component.
...
<CodeSnippets path="button-group-story-subcomponents.md" />

{/* prettier-ignore-end */}

![Subcomponents in Docs Page](../_assets/writing-docs/docspage-subcomponents.png)
...
Override the `docs.page` [parameter](../writing-stories/parameters.mdx) with `null` to remove its contents.

{/* prettier-ignore-start */}

<CodeSnippets path="button-story-disable-docspage-component.md" />

{/* prettier-ignore-end */}

### With MDX documentation

Write your documentation in MDX and update the `docs.page` [parameter](../writing-stories/parameters.mdx) to display it. The `id` of reference follows the pattern: `group-subgroup-...--name`, where the `groups` and `subgroups` are defined as according to the [Grouping Documentation](../writing-stories/naming-components-and-hierarchy.mdx#grouping).
...
Finally write your custom React component and update the `docs.page` [parameter](../writing-stories/parameters.mdx) to render the custom documentation.

{/* prettier-ignore-start */}

<CodeSnippets path="custom-docs-page-with-story.md" />
...
<CodeSnippets path="button-story-remix-docspage.md" />
...
Therefore, we recommend inline rendering where possible. It's the default mode for all the frameworks in which [we support it](../api/frameworks-feature-support.mdx). The one exception is Angular, where it's opt-in.

To toggle the between the two settings, set `docs.inlineStories` in `.storybook/preview.js`. Like most [parameters](../writing-stories/parameters.mdx), you can also toggle at the component or story level:

{/* prettier-ignore-start */}

<CodeSnippets path="storybook-preview-optout-inline.md" />

{/* prettier-ignore-end */}

### Custom inline rendering

If your framework doesn't [support inline rendering](../api/frameworks-feature-support.mdx), you also need to provide a `prepareForInline` function in addition to the `inlineStories` parameter.
...
<CodeSnippets path="storybook-preview-prepareforinline.md" />
