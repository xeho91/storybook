...
<details>
  ...
  * Specify a custom [Babel configuration](../configure/integration/babel.mdx#custom-babel-config) for Storybook. Storybook automatically tries to use your project’s config if it can.
...
<details>
  ...
  Storybook allows you to import CSS files in your components directly. But in some cases you may need to [tweak its Webpack configuration](../builders/webpack.mdx#extendingstorybooks-webpack-config). Angular components require [a special import](../configure/integration/styling-and-css.mdx#importing-css-files).
</details>

<details>
  <summary>Global imported styles</summary>

  If you have global imported styles, create a file called [`.storybook/preview.js`](../configure/index.mdx#configure-story-rendering) and import the styles there. They will be added by Storybook automatically for all stories.
</details>
...
<details>
  ...
  If you're referencing fonts or images from a local directory, you'll need to configure the Storybook script to [serve the static files](../configure/integration/images-and-assets.mdx).
</details>

## Load assets and resources

If you want to [link to static files](../configure/integration/images-and-assets.mdx) in your project or stories (e.g., `/fonts/XYZ.woff`), use the `-s path/to/folder` flag to specify a static folder to serve from when you start up Storybook. To do so, edit the `storybook` and `build-storybook` scripts in `package.json`.
