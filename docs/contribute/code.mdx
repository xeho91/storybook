...
![Example Storybook running](../_assets/contribute/storybook-cra-examples-optimized.png)
...
![Storybook UI](../_assets/contribute/storybook-manager-preview.jpg)
...
If you're modifying part of Storybook's core, or one of the essential addons, there's probably an existing set of stories in the [`official-storybook`](https://github.com/storybookjs/storybook/tree/release-6-5/examples/official-storybook) that documents how the feature is supposed to work. Add your stories there.

If you're modifying something related to a specific framework, the framework will have its own examples in the monorepo. For instance, [`examples/vue-kitchen-sink`](https://github.com/storybookjs/storybook/tree/release-6-5/examples/vue-kitchen-sink) is a natural place to add stories for `@storybook/vue` while [`examples/angular-cli`](https://github.com/storybookjs/storybook/tree/release-6-5/examples/angular-cli) is the place for `@storybook/angular`.
...
<Callout variant="info" icon="💡">
  ...
  Storybook uses [jest](https://jestjs.io/) as part of the testing suite, if you notice that the snapshot tests fail you can re-run and update them with `yarn test -u`.
