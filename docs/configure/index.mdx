...
<CodeSnippets path="storybook-main-default-setup.md" />

{/* prettier-ignore-end */}

The `main.js` configuration file is a [preset](../addons/addon-types.mdx) and, as such, has a powerful interface, but the key fields within it are:
...
* `webpackFinal` - custom [webpack configuration](../builders/webpack.mdx#extending-storybooks-webpack-config).
* `babel` - custom [babel configuration](./babel.mdx).
...
| `babelModeV7`         | Enables the new [Babel configuration](./babel.mdx#v7-mode) mode for Storybook. <br /> `features: { babelModeV7: true }`                                                                                                                |
...
| `previewMdx2`         | Enables experimental support for [MDX 2](../writing-docs/mdx.mdx#mdx-2).<br />`features: { previewMdx2: true }`                                                                                                                        |
...
<CodeSnippets path="storybook-storyloading-with-custom-object.md" />
...
<CodeSnippets path="storybook-storyloading-custom-logic.md" />
...
<CodeSnippets path="storybook-on-demand-story-loading.md" />
...
* [CSF formats](../api/csf.mdx) from version 1 to version 3 are supported. The `storiesOf` construct is not.
...
<CodeSnippets path="storybook-ts-config-babelrc.md" />
...
| `staticDirs`          | Sets a list of directories of [static files](./images-and-assets.mdx#serving-static-files-via-storybook-configuration) to be loaded by Storybook <br /> `staticDirs:['../public']` |
...
This file does not have a specific API but is the place to set [UI options](./features-and-behavior.mdx) and to configure Storybook’s [theme](./theming.mdx).
