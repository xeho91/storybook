...
* Storybook metadata:
  ...
  * [Addons](https://storybook.js.org/addons/) (e.g., [Essentials](../essentials/index.mdx), [Accessibility](../writing-tests/accessibility-testing.mdx)).
  * [Feature flags](./index.mdx#feature-flags) (e.g., `buildStoriesJson`).
...
You may opt-out of the telemetry within your Storybook configuration by setting the `disableTelemetry` configuration element to `true`. For example:
...
Or via the command line with the `--disable-telemetry` flag or the `STORYBOOK_DISABLE_TELEMETRY` environment variable:

{/* prettier-ignore-start */}

<CodeSnippets path="storybook-disable-telemetry-cli.md" />
...
In addition to general usage telemetry, you may also choose to share crash reports. Storybook will then sanitize the error object (removing all user paths) and append it to the telemetry event. To enable crash reporting, you can set the `enableCrashReports` configuration element to `true`. For example:
...
Additionally, you can enable crash reports via the command line with the `--enable-crash-reports` flag or the `STORYBOOK_ENABLE_CRASH_REPORTS` environment variable.

{/* prettier-ignore-start */}

<CodeSnippets path="storybook-telemetry-storybook-enable-crash-reports.md" />
