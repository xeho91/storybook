...
You can use [argTypes](../api/stories/argtypes.mdx) to tell Storybook that an arg to your story should be an action. Usually, it makes sense to do this at the component level (although you can apply it per individual story):
...
Another option is to use a global parameter to match all [argTypes](../api/stories/argtypes.mdx) that match a certain pattern. The following configuration automatically creates actions for each `on` argType (which you can either specify manually or can be [inferred automatically](../api/stories/argtypes.mdx#automatic-argtype-inference)).
...
There are also some older ways to use actions as documented in the [advanced README](https://github.com/storybookjs/storybook/blob/release-6-5/addons/actions/ADVANCED.md).
