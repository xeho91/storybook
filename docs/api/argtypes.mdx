...
The most concrete realization of argTypes is the [Args Table](../writing-docs/doc-block-argstable.mdx) doc block. Each row in the table corresponds to a single argType and the current value of that arg.

![Storybook inferring automatically the argType](../_assets/api/argstable.png)

## Automatic argType inference

If you are using the Storybook [docs](../writing-docs/introduction.mdx) addon (installed by default as part of [essentials](../essentials/introduction.mdx)), then Storybook will infer a set of argTypes for each story based on the `component` specified in the [default export](./csf.mdx#default-export) of the CSF file.
